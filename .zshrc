...
sync()
{
    ...
    if [ $1 = pull ]; then
        rsync -anv --exclude env --delete $OTHER_NODE:unsw ~/unsw
        echo -n "\nDo you wish to commit to these changes (y/n)? "
        while true; do
            read answer
            if [ "$answer" != "${answer#[Yy]}" ]; then
                rsync -av --exclude env --delete $OTHER_NODE:unsw ~/unsw
                echo "\nDone!"
                break
        ...
        rsync -anv --exclude env --delete ~/unsw $OTHER_NODE:unsw
        echo -n "\nDo you wish to push to these changes (y/n)? "
        while true; do
            read answer
            if [ "$answer" != "${answer#[Yy]}" ]; then
                rsync -av --exclude env --delete ~/unsw $OTHER_NODE:unsw
                echo "\nDone!"
                break
            elif [ "$answer" != "${answer#[Nn]}" ]; then
